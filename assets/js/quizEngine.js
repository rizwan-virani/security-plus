...
{ id:"D1-018", domain:1, obj:"1.2", diff:"medium", q:"Which security concept ensures that a sender cannot later deny having sent a digitally signed message?", options:["Confidentiality","Integrity","Non-repudiation","Availability"], answer:2, explain:"<strong>Non-repudiation</strong> prevents a sender from denying an action, and a digital signature provides it by binding the message to the signer's private key.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Confidentiality</b>: confidentiality hides message content from unauthorized parties but does not prove who sent it.</span><span class='qd'><b>Integrity</b>: integrity alone proves the message was not altered but cannot establish the identity of the sender.</span><span class='qd'><b>Availability</b>: availability ensures access to data and has nothing to do with proving authorship.</span>" },
{ id:"D1-019", domain:1, obj:"1.2", diff:"easy", q:"What is the core principle of a Zero Trust architecture?", options:["Trust all traffic inside the corporate perimeter","Never trust, always verify every request","Trust devices once they pass the firewall","Trust users after a single successful login"], answer:1, explain:"Zero Trust assumes no implicit trust and requires continuous verification of every request, captured by <strong>Never trust, always verify every request</strong>.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Trust all traffic inside the corporate perimeter</b>: this is the legacy perimeter model that Zero Trust was created to replace.</span><span class='qd'><b>Trust devices once they pass the firewall</b>: granting implicit trust after the firewall contradicts Zero Trust, which continuously reverifies devices.</span><span class='qd'><b>Trust users after a single successful login</b>: Zero Trust demands ongoing verification rather than trusting a one-time login.</span>" },
{ id:"D1-020", domain:1, obj:"1.2", diff:"hard", q:"In the Zero Trust model, the component that makes the access decision based on policy is the policy decision point. Which component actually ENFORCES that decision on the connection?", options:["Policy engine","Policy administrator","Policy enforcement point","Policy information point"], answer:2, explain:"The <strong>policy enforcement point</strong> enforces the access decision by allowing or blocking the actual connection.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Policy engine</b>: the policy engine evaluates rules and makes the decision, but it does not enforce it on the connection.</span><span class='qd'><b>Policy administrator</b>: the policy administrator communicates the engine's decision to the enforcement point but does not itself pass or block traffic.</span><span class='qd'><b>Policy information point</b>: the policy information point supplies contextual data for the decision and plays no enforcement role.</span>" },
{ id:"D1-021", domain:1, obj:"1.2", diff:"hard", q:"In a Zero Trust deployment, the policy engine and policy administrator make and communicate access decisions. These two components are collectively known as which plane?", options:["Data plane","Control plane","Management plane","Trust plane"], answer:1, explain:"The policy engine and policy administrator together form the <strong>control plane</strong>, which decides and conveys access decisions.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Data plane</b>: the data plane is where the enforcement point actually passes or blocks traffic, not where decisions are made.</span><span class='qd'><b>Management plane</b>: the management plane handles administration and configuration of devices, which is not the Zero Trust decision-making grouping.</span><span class='qd'><b>Trust plane</b>: trust plane is not a defined component of the Zero Trust architecture.</span>" },
...
{ id:"D1-023", domain:1, obj:"1.2", diff:"medium", q:"Which deception technology consists of fake credentials or files placed to alert defenders when an attacker accesses them?", options:["Honeynet","Honeytoken","Honeypot","Sinkhole"], answer:1, explain:"A <strong>Honeytoken</strong> is bogus data such as fake credentials or files that triggers an alert when an attacker accesses or uses it.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Honeynet</b>: a honeynet is an entire decoy network of multiple honeypots, not a single fake credential or file.</span><span class='qd'><b>Honeypot</b>: a honeypot is a complete decoy system, whereas a honeytoken is a small piece of planted bait data.</span><span class='qd'><b>Sinkhole</b>: a sinkhole redirects malicious traffic to a controlled destination and is not planted bait data that triggers an alert.</span>" },
{ id:"D1-024", domain:1, obj:"1.2", diff:"hard", q:"A security team wants to detect lateral movement by attackers who have already breached the perimeter, without exposing real production data. Which deception approach BEST fits?", options:["Deploy a single internet-facing honeypot in the DMZ","Place honeytokens and internal decoy hosts within the production segments","Encrypt all internal databases with AES-256","Require MFA for all external logins"], answer:1, explain:"Internal <strong>Place honeytokens and internal decoy hosts within the production segments</strong> reveal attackers already moving laterally without exposing real data.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Deploy a single internet-facing honeypot in the DMZ</b>: a perimeter honeypot catches external probing but cannot detect movement already inside the network.</span><span class='qd'><b>Encrypt all internal databases with AES-256</b>: encryption protects data confidentiality but provides no detection of lateral movement.</span><span class='qd'><b>Require MFA for all external logins</b>: external MFA strengthens perimeter authentication and does nothing to spot an attacker already past it.</span>" },
{ id:"D1-025", domain:1, obj:"1.2", diff:"medium", q:"What is the primary purpose of a gap analysis in a security program?", options:["Encrypt data in transit","Compare the current security state to a desired state","Authenticate privileged users","Recover systems after a disaster"], answer:1, explain:"A <strong>Compare the current security state to a desired state</strong> defines a gap analysis, which measures the difference against a target framework to guide remediation.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Encrypt data in transit</b>: encrypting traffic is a technical control, not an assessment comparing security posture to a goal.</span><span class='qd'><b>Authenticate privileged users</b>: authenticating users verifies identity and is unrelated to measuring program maturity.</span><span class='qd'><b>Recover systems after a disaster</b>: disaster recovery restores operations after an incident, not before, and is not a gap assessment.</span>" },
...
{ id:"D1-027", domain:1, obj:"1.2", diff:"medium", q:"A mantrap (access control vestibule) installed at a data center entrance is intended primarily to prevent which physical security threat?", options:["Tailgating/piggybacking","Shoulder surfing","Eavesdropping on wireless signals","SQL injection"], answer:0, explain:"An access control vestibule (<strong>Tailgating/piggybacking</strong>) permits one person through at a time, preventing an unauthorized person from following an authorized one.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Shoulder surfing</b>: shoulder surfing is observing a person entering credentials, which a vestibule does not address.</span><span class='qd'><b>Eavesdropping on wireless signals</b>: wireless eavesdropping intercepts radio traffic and is a logical, not physical-entry, threat.</span><span class='qd'><b>SQL injection</b>: SQL injection is an application attack against a database and is entirely unrelated to physical entry control.</span>" },
{ id:"D1-028", domain:1, obj:"1.2", diff:"easy", q:"Which physical control uses unique biological characteristics, such as a fingerprint, to grant access?", options:["Proximity card","Biometric reader","PIN pad","Bollard"], answer:1, explain:"A <strong>Biometric reader</strong> authenticates based on unique physical traits such as fingerprints or retinas, which is something you are.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Proximity card</b>: a proximity card is something you have, not a biological characteristic.</span><span class='qd'><b>PIN pad</b>: a PIN is something you know, relying on memorized digits rather than a physical trait.</span><span class='qd'><b>Bollard</b>: a bollard is a physical barrier that stops vehicles and performs no identity authentication.</span>" },
...
{ id:"D1-051", domain:1, obj:"1.4", diff:"hard", q:"An organization needs to secure many subdomains such as mail.example.com and shop.example.com under one certificate without listing each individually. Which certificate type is BEST?", options:["Wildcard certificate","Self-signed certificate","Root certificate","Code-signing certificate"], answer:0, explain:"A <strong>Wildcard certificate</strong> such as *.example.com secures all first-level subdomains under one certificate without listing each individually.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Self-signed certificate</b>: a self-signed certificate is not trusted by clients and does not address covering many subdomains.</span><span class='qd'><b>Root certificate</b>: a root certificate anchors a CA's trust chain and is not deployed on individual web servers.</span><span class='qd'><b>Code-signing certificate</b>: a code-signing certificate verifies software authorship, not subdomain web traffic.</span>" },
{ id:"D1-052", domain:1, obj:"1.4", diff:"hard", q:"A certificate must validate multiple distinct domain names, such as example.com and example.net, within a single certificate. Which certificate field or type provides this?", options:["Wildcard certificate","Subject Alternative Name (SAN) certificate","Certificate signing request","Online Certificate Status Protocol"], answer:1, explain:"A <strong>Subject Alternative Name (SAN) certificate</strong> lists multiple distinct domain names, such as example.com and example.net, within a single certificate.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Wildcard certificate</b>: a wildcard covers subdomains of one base domain, not different top-level domains.</span><span class='qd'><b>Certificate signing request</b>: a CSR is the request submitted to obtain a certificate, not a certificate covering multiple names.</span><span class='qd'><b>Online Certificate Status Protocol</b>: OCSP checks revocation status and does not bind multiple domain names into one certificate.</span>" },
{ id:"D1-053", domain:1, obj:"1.4", diff:"medium", q:"What is generated and sent to a certificate authority to request a new certificate, containing the applicant's public key and identity information?", options:["Certificate revocation list","Certificate signing request","Private key","Key escrow record"], answer:1, explain:"A <strong>Certificate signing request</strong> contains the applicant's public key and identity information and is sent to the CA to request a certificate.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Certificate revocation list</b>: a CRL lists revoked certificates and is not used to request a new one.</span><span class='qd'><b>Private key</b>: the private key is kept secret by the applicant and is never sent to the CA.</span><span class='qd'><b>Key escrow record</b>: a key escrow record is a stored backup of keys for recovery, not a certificate request.</span>" },
{ id:"D1-054", domain:1, obj:"1.4", diff:"hard", q:"An e-commerce site wants the highest level of identity assurance, displaying the verified organization's name after a rigorous vetting process. Which certificate validation level BEST meets this?", options:["Domain Validation (DV)","Organization Validation (OV)","Extended Validation (EV)","Self-signed"], answer:2, explain:"<strong>Extended Validation (EV)</strong> requires the most rigorous vetting and provides the highest identity assurance, displaying the verified organization name.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Domain Validation (DV)</b>: DV only confirms control of the domain with no organizational vetting.</span><span class='qd'><b>Organization Validation (OV)</b>: OV verifies the organization but uses a less rigorous process than EV and offers lower assurance.</span><span class='qd'><b>Self-signed</b>: a self-signed certificate involves no third-party vetting and provides no identity assurance at all.</span>" },
{ id:"D1-055", domain:1, obj:"1.4", diff:"easy", q:"What characteristic of a blockchain makes its ledger resistant to tampering?", options:["Each block is encrypted with a single symmetric key","Each block contains the hash of the previous block","All data is stored on one central server","Records can be silently edited by administrators"], answer:1, explain:"<strong>Each block contains the hash of the previous block</strong>, so altering any block invalidates every subsequent hash across the distributed ledger.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Each block is encrypted with a single symmetric key</b>: blockchain integrity comes from chained hashing, not from encrypting blocks with a shared key.</span><span class='qd'><b>All data is stored on one central server</b>: a blockchain is distributed across many nodes, and central storage would remove its tamper resistance.</span><span class='qd'><b>Records can be silently edited by administrators</b>: silent editing is the opposite of blockchain, which makes changes detectable through the hash chain.</span>" },
{ id:"D1-056", domain:1, obj:"1.4", diff:"medium", q:"Replacing sensitive data such as a credit card number with a non-sensitive substitute value that has no exploitable meaning is called:", options:["Tokenization","Key stretching","Steganography","Salting"], answer:0, explain:"<strong>Tokenization</strong> substitutes sensitive data with a meaningless token that carries no exploitable relationship to the original, mapped back only in a secure vault.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Key stretching</b>: key stretching slows password hashing and does not replace data with a substitute value.</span><span class='qd'><b>Steganography</b>: steganography hides a message inside another file rather than swapping data for a token.</span><span class='qd'><b>Salting</b>: salting adds a random value before hashing passwords and does not produce a reversible substitute value.</span>" },
...
SECPLUS.questions.push(
  ...
  { id:"D2-015", domain:2, obj:"2.2", diff:"medium", q:"A vulnerability is introduced when a trusted hardware supplier ships components with pre-installed malicious firmware. Which threat vector does this represent?", options:["Phishing vector","Supply chain vector","Wireless vector","Default credential vector"], answer:1, explain:"<strong>Supply chain vector</strong> describes compromise introduced through a trusted hardware supplier shipping components with malicious firmware.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Phishing vector</b>: phishing tricks a user into acting, but here the compromise is built into delivered hardware.</span><span class='qd'><b>Wireless vector</b>: no radio interception is involved; the malware arrives pre-installed in the product.</span><span class='qd'><b>Default credential vector</b>: this involves unchanged factory logins, not malicious firmware embedded by the supplier.</span>" },
  { id:"D2-016", domain:2, obj:"2.2", diff:"medium", q:"Which scenario BEST illustrates an unsecured network as a threat vector?", options:["A locked server room with badge access","An open wireless guest network with no segmentation from corporate resources","An encrypted VPN tunnel","A firewall with default-deny rules"], answer:1, explain:"<strong>An open wireless guest network with no segmentation from corporate resources</strong> illustrates an unsecured network that lets attackers reach internal systems directly.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>A locked server room with badge access</b>: this is a strong physical control, the opposite of an unsecured network.</span><span class='qd'><b>An encrypted VPN tunnel</b>: a VPN protects traffic in transit, reducing rather than creating network exposure.</span><span class='qd'><b>A firewall with default-deny rules</b>: default-deny is a secure configuration that blocks unauthorized traffic, not an unsecured network.</span>" },
  { id:"D2-017", domain:2, obj:"2.2", diff:"hard", q:"A company's exposed RDP service on the public internet is repeatedly targeted. Which BEST describes how this expands the attack surface?", options:["It reduces the attack surface by centralizing access","An open remote-access service vector exposes authentication to brute-force and exploitation","It is only a concern for wireless clients","It eliminates the need for patching"], answer:1, explain:"<strong>An open remote-access service vector exposes authentication to brute-force and exploitation</strong> correctly describes how an internet-facing RDP service enlarges the attack surface.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>It reduces the attack surface by centralizing access</b>: exposing RDP to the internet adds an entry point, it does not shrink the attack surface.</span><span class='qd'><b>It is only a concern for wireless clients</b>: internet-exposed RDP is reachable by anyone online, not just wireless users.</span><span class='qd'><b>It eliminates the need for patching</b>: exposed services need patching more urgently, since unpatched flaws are actively targeted.</span>" },
  { id:"D2-018", domain:2, obj:"2.2", diff:"easy", q:"Which of the following is an example of a message-based threat vector other than email?", options:["SMS text (smishing)","USB flash drive","Vendor firmware","Open Wi-Fi"], answer:0, explain:"<strong>SMS text (smishing)</strong> is a message-based vector that uses text messages, alongside email, to lure victims.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>USB flash drive</b>: a USB drive is a removable-device vector, not a messaging channel.</span><span class='qd'><b>Vendor firmware</b>: malicious vendor firmware is a supply chain vector, not a message sent to a user.</span><span class='qd'><b>Open Wi-Fi</b>: open Wi-Fi is an unsecured network or wireless vector, not a message-based one.</span>" },
  { id:"D2-019", domain:2, obj:"2.2", diff:"hard", q:"An attacker exploits a misconfigured API endpoint that returns excessive data. Which attack surface concept does this MOST directly illustrate?", options:["Physical access","Application/software attack surface via exposed interfaces","Wireless interception","Removable media"], answer:1, explain:"<strong>Application/software attack surface via exposed interfaces</strong> is illustrated by a misconfigured API returning excessive data, a weakness in the software interface.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Physical access</b>: physical access concerns touching the hardware, not abusing a remote API.</span><span class='qd'><b>Wireless interception</b>: wireless interception captures radio traffic, but the flaw here is the API logic, not signal capture.</span><span class='qd'><b>Removable media</b>: removable media involves USB or disk-based delivery, unrelated to an exposed API endpoint.</span>" },
  { id:"D2-020", domain:2, obj:"2.2", diff:"medium", q:"Which threat vector is being used when an attacker connects to a system via an unpatched, internet-facing web application?", options:["Removable device vector","Vulnerable software vector","Bluetooth vector","Default credential vector"], answer:1, explain:"<strong>Vulnerable software vector</strong> is used when an attacker exploits known flaws in an unpatched, internet-facing web application.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Removable device vector</b>: this requires physical media like USB, not exploitation of an exposed application.</span><span class='qd'><b>Bluetooth vector</b>: Bluetooth is a short-range wireless attack path, not a route to a web app over the internet.</span><span class='qd'><b>Default credential vector</b>: this abuses unchanged factory passwords, whereas the scenario exploits an unpatched software flaw.</span>" },
  { id:"D2-021", domain:2, obj:"2.2", diff:"hard", q:"A penetration tester gains access by exploiting a system still configured with the manufacturer's factory login. Which threat vector is this, and why is it dangerous?", options:["Phishing; it tricks users","Default credentials; they are publicly documented and rarely changed","Supply chain; the vendor was breached","Wireless; the signal was intercepted"], answer:1, explain:"<strong>Default credentials; they are publicly documented and rarely changed</strong> correctly identifies the vector and why factory logins remain dangerous.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Phishing; it tricks users</b>: phishing manipulates a person, but here the tester simply used a known factory login, no deception involved.</span><span class='qd'><b>Supply chain; the vendor was breached</b>: nothing indicates a vendor compromise; the flaw is an unchanged default password.</span><span class='qd'><b>Wireless; the signal was intercepted</b>: no radio interception occurred; access came from the factory credentials.</span>" },
  ...
  { id:"D2-023", domain:2, obj:"2.2", diff:"easy", q:"Which of the following BEST describes an 'attack surface'?", options:["The total of all points where an attacker could attempt entry","Only the company firewall","A list of patched vulnerabilities","The encryption keys in use"], answer:0, explain:"<strong>The total of all points where an attacker could attempt entry</strong> is the definition of the attack surface, summing every interface, service, and account exposed.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Only the company firewall</b>: the firewall is a single control, not the full set of entry points across the environment.</span><span class='qd'><b>A list of patched vulnerabilities</b>: patched flaws describe remediation status, not the breadth of exposed entry points.</span><span class='qd'><b>The encryption keys in use</b>: keys protect data confidentiality and are not a measure of exposed attack paths.</span>" },
  { id:"D2-024", domain:2, obj:"2.2", diff:"medium", q:"An organization allows employees to use personal devices that connect directly to corporate email without management controls. Which threat vector concern is MOST relevant?", options:["Unmanaged endpoints expanding the attack surface","Reduced supply chain risk","Eliminated need for MFA","Stronger network segmentation"], answer:0, explain:"<strong>Unmanaged endpoints expanding the attack surface</strong> is the key concern when personal devices connect without management controls, adding unmonitored entry points.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Reduced supply chain risk</b>: personal devices have no bearing on vendor supply chain risk and add risk rather than reducing it.</span><span class='qd'><b>Eliminated need for MFA</b>: unmanaged devices make strong authentication more important, not unnecessary.</span><span class='qd'><b>Stronger network segmentation</b>: unmanaged devices on corporate email weaken control, they do not improve segmentation.</span>" },
  { id:"D2-025", domain:2, obj:"2.3", diff:"easy", q:"Which vulnerability type occurs when a program writes more data to a memory region than it was allocated?", options:["Race condition","Buffer overflow","SQL injection","Misconfiguration"], answer:1, explain:"<strong>Buffer overflow</strong> occurs when a program writes more data than a buffer holds, overwriting adjacent memory and potentially enabling code execution.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Race condition</b>: a race condition is a timing flaw between concurrent operations, not a memory-size violation.</span><span class='qd'><b>SQL injection</b>: SQL injection inserts malicious queries through input, it does not overflow a memory buffer.</span><span class='qd'><b>Misconfiguration</b>: a misconfiguration is an insecure setting, not a programmatic overrun of allocated memory.</span>" },
  ...
  { id:"D2-039", domain:2, obj:"2.3", diff:"easy", q:"What is the BEST general definition of a 'vulnerability' in security terms?", options:["An active attack in progress","A weakness that could be exploited by a threat","A control that blocks attacks","A confirmed data breach"], answer:1, explain:"<strong>A weakness that could be exploited by a threat</strong> is the standard definition of a vulnerability in security terms.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>An active attack in progress</b>: an attack in progress is an incident or exploit event, not the underlying weakness.</span><span class='qd'><b>A control that blocks attacks</b>: a control is a safeguard, the opposite of a weakness.</span><span class='qd'><b>A confirmed data breach</b>: a breach is a realized impact, not the latent weakness that allowed it.</span>" },
  { id:"D2-040", domain:2, obj:"2.3", diff:"hard", q:"A penetration test reveals that an application trusts data from a client-side check without re-validating it on the server. Which vulnerability does this BEST represent?", options:["Improper input validation / client-side trust","Cryptographic downgrade","DNS poisoning","Resource exhaustion"], answer:0, explain:"<strong>Improper input validation / client-side trust</strong> describes relying on a client-side check without re-validating on the server, since clients can be manipulated.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Cryptographic downgrade</b>: a downgrade forces weaker encryption, unrelated to trusting unverified client data.</span><span class='qd'><b>DNS poisoning</b>: DNS poisoning corrupts name resolution, not server trust of client-side validation.</span><span class='qd'><b>Resource exhaustion</b>: resource exhaustion depletes system capacity for a denial of service, not a validation trust flaw.</span>" },
  { id:"D2-041", domain:2, obj:"2.4", diff:"medium", q:"Which type of malware encrypts a victim's files and demands payment for the decryption key?", options:["Spyware","Ransomware","Rootkit","Worm"], answer:1, explain:"<strong>Ransomware</strong> encrypts a victim's files and demands payment, usually in cryptocurrency, for the decryption key.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Spyware</b>: spyware secretly collects information, it does not encrypt files or demand a ransom.</span><span class='qd'><b>Rootkit</b>: a rootkit hides its presence to maintain stealthy access, not to extort payment via encryption.</span><span class='qd'><b>Worm</b>: a worm self-replicates across networks, which is about propagation, not file encryption for ransom.</span>" },
  ...
  { id:"D2-043", domain:2, obj:"2.4", diff:"medium", q:"A program appears to be a legitimate utility but secretly opens a backdoor when run. Which malware type is this?", options:["Worm","Trojan","Rootkit","Bloatware"], answer:1, explain:"<strong>Trojan</strong> disguises itself as legitimate software while performing malicious actions, such as opening a backdoor, when run.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Worm</b>: a worm self-replicates across networks rather than masquerading as a useful utility.</span><span class='qd'><b>Rootkit</b>: a rootkit focuses on hiding its presence to keep access, not on appearing to be a legitimate program.</span><span class='qd'><b>Bloatware</b>: bloatware is unwanted preinstalled software that is annoying but not disguised malware opening a backdoor.</span>" },
  { id:"D2-044", domain:2, obj:"2.4", diff:"hard", q:"An incident responder finds malware that hooks the operating system kernel to hide its files and processes from administrators. Which indicator and malware type are MOST consistent?", options:["High CPU only; spyware","Hidden processes/files via kernel hooks; rootkit","Mass file encryption; ransomware","Self-propagation; worm"], answer:1, explain:"<strong>Hidden processes/files via kernel hooks; rootkit</strong> matches malware that hooks the OS kernel to conceal itself from administrators.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>High CPU only; spyware</b>: spyware steals information and high CPU alone does not indicate kernel-level concealment.</span><span class='qd'><b>Mass file encryption; ransomware</b>: ransomware encrypts files for ransom, it does not hook the kernel to hide processes.</span><span class='qd'><b>Self-propagation; worm</b>: a worm's signature is spreading across hosts, not hiding files through kernel hooks.</span>" },
  { id:"D2-045", domain:2, obj:"2.4", diff:"medium", q:"Which password attack systematically tries every possible character combination until the correct one is found?", options:["Dictionary attack","Brute-force attack","Pass-the-hash","Rainbow table"], answer:1, explain:"<strong>Brute-force attack</strong> systematically tries every possible character combination until the correct password is found, exhaustive but slow.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Dictionary attack</b>: a dictionary attack tries a curated list of likely words, not every possible combination.</span><span class='qd'><b>Pass-the-hash</b>: pass-the-hash reuses a captured hash to authenticate, it does not guess characters.</span><span class='qd'><b>Rainbow table</b>: a rainbow table looks up precomputed hashes to reverse them, rather than trying combinations live.</span>" },
...
SECPLUS.questions.push(
  ...
  { id:"D2-071", domain:2, obj:"2.4", diff:"medium", q:"Users report that visiting their bank's correct URL still lands them on a fraudulent site. Investigation shows the local hosts file was modified. Which attack is MOST likely?", options:["Phishing email","Pharming","SQL injection","Brute force"], answer:1, explain:"<strong>Pharming</strong> redirects users from a legitimate URL to a malicious site, here by modifying the local hosts file so the correct address resolves to the attacker.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Phishing email</b>: phishing relies on the user clicking a deceptive link, but here the user typed the correct URL and was still redirected.</span><span class='qd'><b>SQL injection</b>: SQL injection manipulates a backend database, it does not alter name resolution or the hosts file.</span><span class='qd'><b>Brute force</b>: brute force guesses credentials, it has nothing to do with redirecting a correct URL to a fake site.</span>" },
  { id:"D2-072", domain:2, obj:"2.4", diff:"medium", q:"An attacker forces a TLS connection to fall back from TLS 1.3 to SSL 3.0 to exploit a weakness. This is a(n):", options:["Replay attack","Downgrade attack","Collision attack","On-path attack"], answer:1, explain:"<strong>Downgrade attack</strong> forces parties to negotiate a weaker, vulnerable protocol or cipher, such as falling back from TLS 1.3 to SSL 3.0, than they would normally use.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Replay attack</b>: a replay attack resends captured valid data, it does not force a weaker protocol version.</span><span class='qd'><b>Collision attack</b>: a collision attack finds two inputs with the same hash, unrelated to negotiating a protocol version.</span><span class='qd'><b>On-path attack</b>: an on-path attack may enable a downgrade, but the specific act of forcing the weaker protocol is itself the downgrade attack.</span>" },
  { id:"D2-073", domain:2, obj:"2.4", diff:"medium", q:"Two different inputs produce the same hash value, undermining the integrity guarantee of the algorithm. This describes a:", options:["Birthday paradox only","Collision attack","Salting failure","Key stretching flaw"], answer:1, explain:"<strong>Collision attack</strong> finds two distinct inputs that hash to the same value, breaking the integrity assurance of the hash function.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Birthday paradox only</b>: the birthday paradox is the probability principle that makes collisions feasible, but the exploitation itself is the collision attack.</span><span class='qd'><b>Salting failure</b>: salting concerns password storage uniqueness, not two arbitrary inputs producing the same hash.</span><span class='qd'><b>Key stretching flaw</b>: key stretching slows brute-force guessing of passwords, it is unrelated to two inputs sharing a hash value.</span>" },
  ...
  { id:"D2-102", domain:2, obj:"2.5", diff:"easy", q:"Granting users only the permissions required to perform their job is the principle of:", options:["Least privilege","Defense in depth","Separation of duties","Implicit deny"], answer:0, explain:"<strong>Least privilege</strong> limits each account's access to the minimum needed, reducing the impact of compromise or misuse.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Defense in depth</b>: defense in depth layers multiple controls, it is broader than restricting a user's permissions to job needs.</span><span class='qd'><b>Separation of duties</b>: separation of duties splits a sensitive task across people to prevent fraud, not limit one account's permissions.</span><span class='qd'><b>Implicit deny</b>: implicit deny blocks anything not explicitly allowed, a rule-processing default rather than the minimum-permission principle.</span>" },
  { id:"D2-103", domain:2, obj:"2.5", diff:"easy", q:"Applying vendor updates to fix known software vulnerabilities is known as:", options:["Hardening","Patching","Isolation","Monitoring"], answer:1, explain:"<strong>Patching</strong> applies vendor updates to remediate known vulnerabilities before attackers can exploit them.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Hardening</b>: hardening reduces attack surface by disabling services and accounts, it is broader than applying specific vendor fixes.</span><span class='qd'><b>Isolation</b>: isolation quarantines a host from the network, it does not apply software updates.</span><span class='qd'><b>Monitoring</b>: monitoring observes activity to detect issues, it does not remediate vulnerabilities with updates.</span>" },
  { id:"D2-104", domain:2, obj:"2.5", diff:"easy", q:"Disabling unnecessary services, closing unused ports, and removing default accounts is part of:", options:["System hardening","Network segmentation","Data decommissioning","Encryption at rest"], answer:0, explain:"<strong>System hardening</strong> reduces a system's attack surface by removing unneeded services, ports, accounts, and software.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Network segmentation</b>: segmentation isolates the network into zones, it does not disable services or remove default accounts on a host.</span><span class='qd'><b>Data decommissioning</b>: decommissioning sanitizes media when retiring systems, not disable services on an active host.</span><span class='qd'><b>Encryption at rest</b>: encryption at rest protects stored data confidentiality, it does not close ports or remove unneeded services.</span>" },
...
SECPLUS.questions.push(
  ...
  { id:"D3-005", domain:3, obj:"3.1", diff:"medium", q:"Which characteristic most distinguishes a microservices architecture from a traditional monolithic application?", options:["It can only run on physical hardware","Functionality is decomposed into small, independently deployable services","It eliminates the need for any network communication","It requires a single shared database for all components"], answer:1, explain:"<strong>Functionality is decomposed into small, independently deployable services</strong> is the defining trait of microservices, which communicate over APIs and can be scaled and released separately.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>It can only run on physical hardware</b>: microservices commonly run in containers and VMs, not exclusively on physical hardware.</span><span class='qd'><b>It eliminates the need for any network communication</b>: microservices rely heavily on network calls between services, increasing rather than removing network communication.</span><span class='qd'><b>It requires a single shared database for all components</b>: microservices favor separate data stores per service, which is the opposite of a single shared database.</span>" },
  { id:"D3-006", domain:3, obj:"3.1", diff:"medium", q:"A Type 1 hypervisor differs from a Type 2 hypervisor in that it:", options:["Runs as an application on top of a host operating system","Runs directly on the physical hardware (bare metal)","Can only virtualize a single guest at a time","Is used exclusively for desktop virtualization"], answer:1, explain:"<strong>Runs directly on the physical hardware (bare metal)</strong> defines a Type 1 hypervisor, which is more efficient and offers stronger isolation than a Type 2 hypervisor.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Runs as an application on top of a host operating system</b>: that describes a Type 2 hypervisor, not a Type 1.</span><span class='qd'><b>Can only virtualize a single guest at a time</b>: both hypervisor types can host many guests simultaneously.</span><span class='qd'><b>Is used exclusively for desktop virtualization</b>: desktop virtualization is a typical Type 2 use case, while Type 1 hypervisors run enterprise server workloads.</span>" },
  { id:"D3-007", domain:3, obj:"3.1", diff:"medium", q:"VM sprawl introduces security risk primarily because:", options:["Virtual machines run faster than physical servers","Unmanaged and unpatched VMs accumulate, expanding the attack surface","Hypervisors cannot enforce any isolation","All VMs must share one IP address"], answer:1, explain:"<strong>Unmanaged and unpatched VMs accumulate, expanding the attack surface</strong> is the core risk of VM sprawl, since forgotten machines become unpatched entry points.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Virtual machines run faster than physical servers</b>: relative speed is irrelevant to sprawl, which is about uncontrolled proliferation, not performance.</span><span class='qd'><b>Hypervisors cannot enforce any isolation</b>: hypervisors do enforce isolation between guests, so this statement is simply false.</span><span class='qd'><b>All VMs must share one IP address</b>: each VM has its own network identity, and shared addressing is not a property of sprawl.</span>" },
  ...
  { id:"D3-025", domain:3, obj:"3.2", diff:"medium", q:"A jump server (jump box) is best described as:", options:["A server that automatically reboots failed services","A hardened, monitored host used as a controlled pivot point to access sensitive network segments","A backup power supply","A load balancer for web traffic"], answer:1, explain:"<strong>A hardened, monitored host used as a controlled pivot point to access sensitive network segments</strong> defines a jump server, providing a single audited entry path.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>A server that automatically reboots failed services</b>: automatic service restart is a monitoring or orchestration function, not a jump box.</span><span class='qd'><b>A backup power supply</b>: backup power is provided by a UPS or generator, unrelated to administrative access pivoting.</span><span class='qd'><b>A load balancer for web traffic</b>: a load balancer distributes requests across servers, not a controlled access pivot.</span>" },
  { id:"D3-026", domain:3, obj:"3.2", diff:"medium", q:"Which statement correctly contrasts IPsec and TLS for securing communications?", options:["IPsec secures traffic at the network layer; TLS secures at the transport/session layer","TLS only works for email and IPsec only for web browsing","Neither provides encryption","IPsec cannot create VPN tunnels"], answer:0, explain:"<strong>IPsec secures traffic at the network layer; TLS secures at the transport/session layer</strong> correctly contrasts the two, with IPsec common for site-to-site VPNs and TLS for application sessions like HTTPS.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>TLS only works for email and IPsec only for web browsing</b>: both protocols secure many traffic types, and these specific restrictions are inaccurate.</span><span class='qd'><b>Neither provides encryption</b>: both IPsec and TLS provide encryption, so this is plainly false.</span><span class='qd'><b>IPsec cannot create VPN tunnels</b>: IPsec is a primary protocol for building VPN tunnels, so this is the opposite of the truth.</span>" },
  { id:"D3-027", domain:3, obj:"3.2", diff:"medium", q:"Port security on a switch is commonly used to:", options:["Increase the speed of all ports","Restrict which MAC addresses may connect to a given port","Encrypt all frames on the wire","Replace the need for VLANs"], answer:1, explain:"<strong>Restrict which MAC addresses may connect to a given port</strong> is what port security does, helping prevent unauthorized devices and MAC flooding.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Increase the speed of all ports</b>: port security is an access control feature and does not affect link speed.</span><span class='qd'><b>Encrypt all frames on the wire</b>: frame encryption is provided by protocols like MACsec, not by MAC-based port security.</span><span class='qd'><b>Replace the need for VLANs</b>: VLANs segment broadcast domains, a separate function port security does not replace.</span>" },
  ...
  { id:"D3-041", domain:3, obj:"3.2", diff:"easy", q:"Which device is BEST suited to evenly distribute incoming web requests across multiple backend servers?", options:["Load balancer","Tape library","Print server","KVM switch"], answer:0, explain:"<strong>Load balancer</strong> distributes incoming web requests across a pool of backend servers to improve availability, scalability, and performance.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Tape library</b>: a tape library stores backup media and performs no request distribution.</span><span class='qd'><b>Print server</b>: a print server manages print jobs, not web request distribution.</span><span class='qd'><b>KVM switch</b>: a KVM switch shares a keyboard, video, and mouse across machines, unrelated to balancing network traffic.</span>" },
  { id:"D3-042", domain:3, obj:"3.2", diff:"hard", q:"An organization wants TLS-encrypted traffic inspected for malware before it reaches internal users, while preserving the ability to log decisions. Which capability MUST the security device support?", options:["Plaintext-only inspection","TLS decryption (SSL/TLS inspection) with re-encryption","Disabling all encryption on the network","Routing all traffic around the inspection point"], answer:1, explain:"<strong>TLS decryption (SSL/TLS inspection) with re-encryption</strong> is required to scan encrypted traffic for malware, decrypting, inspecting, then re-encrypting the flow.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Plaintext-only inspection</b>: plaintext-only inspection cannot read TLS-encrypted content, so threats stay hidden.</span><span class='qd'><b>Disabling all encryption on the network</b>: disabling encryption destroys confidentiality and is not a viable inspection method.</span><span class='qd'><b>Routing all traffic around the inspection point</b>: bypassing the inspection device means no scanning occurs at all.</span>" },
  { id:"D3-043", domain:3, obj:"3.2", diff:"medium", q:"A stateful firewall improves on a simple stateless packet filter by:", options:["Ignoring the connection state entirely","Tracking the state of active connections and allowing return traffic for established sessions","Only filtering based on time of day","Blocking all outbound traffic permanently"], answer:1, explain:"<strong>Tracking the state of active connections and allowing return traffic for established sessions</strong> is how a stateful firewall improves on a stateless packet filter.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Ignoring the connection state entirely</b>: ignoring state describes a stateless filter, the very thing being improved upon.</span><span class='qd'><b>Only filtering based on time of day</b>: time-based rules are a niche feature, not the defining stateful improvement.</span><span class='qd'><b>Blocking all outbound traffic permanently</b>: blocking all outbound traffic would break connectivity and is not what statefulness means.</span>" },
...
SECPLUS.questions.push(
  ...
  { id:"D3-053", domain:3, obj:"3.3", diff:"hard", q:"A retailer must store payment data but wants to remove cardholder data from most of its environment to reduce PCI DSS scope while still supporting recurring billing. Which approach is the BEST fit?", options:["Encrypt the card numbers with AES and keep them in every application","Tokenize the card numbers and store tokens in applications","Hash the card numbers with SHA-256 in every system","Mask the card numbers everywhere they appear"], answer:1, explain:"<strong>Tokenize the card numbers and store tokens in applications</strong> removes actual cardholder data from systems, shrinking PCI scope while the token vault still supports recurring billing.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Encrypt the card numbers with AES and keep them in every application</b>: encrypted card data is still in scope as cardholder data in every application, so it does not shrink scope.</span><span class='qd'><b>Hash the card numbers with SHA-256 in every system</b>: hashing is one-way, so the card number cannot be recovered for recurring billing.</span><span class='qd'><b>Mask the card numbers everywhere they appear</b>: masking only hides display digits while the underlying sensitive data often remains, so it does not remove data from scope.</span>" },
  { id:"D3-054", domain:3, obj:"3.3", diff:"hard", q:"An analyst must guarantee that a downloaded file has not been altered in transit but does NOT need confidentiality. Which control MOST directly meets this need?", options:["Encrypting the file with AES","Comparing a published SHA-256 hash of the file","Tokenizing the file contents","Applying full-disk encryption"], answer:1, explain:"<strong>Comparing a published SHA-256 hash of the file</strong> verifies integrity by detecting any change, which is exactly the requirement without needing confidentiality.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Encrypting the file with AES</b>: AES provides confidentiality, not the integrity verification the scenario requires.</span><span class='qd'><b>Tokenizing the file contents</b>: tokenization substitutes sensitive values and does not confirm a file is unaltered.</span><span class='qd'><b>Applying full-disk encryption</b>: full-disk encryption protects stored data confidentiality but does not detect tampering in transit.</span>" },
  ...
  { id:"D3-061", domain:3, obj:"3.3", diff:"medium", q:"In public key infrastructure, which entity is responsible for issuing and digitally signing certificates?", options:["Registration authority","Certificate authority","Key escrow agent","Validation authority"], answer:1, explain:"<strong>Certificate authority</strong> issues and digitally signs certificates, binding a public key to an identity in a PKI.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Registration authority</b>: the RA verifies requester identity but does not sign or issue certificates.</span><span class='qd'><b>Key escrow agent</b>: an escrow agent stores key copies for recovery and does not issue certificates.</span><span class='qd'><b>Validation authority</b>: a validation authority confirms certificate status, such as via OCSP, rather than issuing certificates.</span>" },
  { id:"D3-062", domain:3, obj:"3.3", diff:"medium", q:"What is the role of a registration authority (RA) in a PKI?", options:["It signs all issued certificates","It verifies the identity of requesters before the CA issues a certificate","It stores private keys for recovery","It revokes certificates automatically"], answer:1, explain:"<strong>It verifies the identity of requesters before the CA issues a certificate</strong> is the RA's role, handling vetting on behalf of the CA.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>It signs all issued certificates</b>: signing certificates is the CA's job, not the RA's.</span><span class='qd'><b>It stores private keys for recovery</b>: key recovery storage is the function of key escrow, not the RA.</span><span class='qd'><b>It revokes certificates automatically</b>: revocation is managed through the CA and CRL/OCSP, not automatically by the RA.</span>" },
  { id:"D3-063", domain:3, obj:"3.3", diff:"easy", q:"Which file does an entity generate and submit to a CA to request a digital certificate?", options:["CRL","CSR","OCSP response","HSM token"], answer:1, explain:"<strong>CSR</strong> (certificate signing request) contains the applicant's public key and identifying information submitted to the CA to request a certificate.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>CRL</b>: a certificate revocation list publishes revoked certificates and is produced by the CA, not submitted to request one.</span><span class='qd'><b>OCSP response</b>: an OCSP response reports a certificate's revocation status, not a request for issuance.</span><span class='qd'><b>HSM token</b>: an HSM is hardware for key protection, not a file submitted to request a certificate.</span>" },
  ...
  { id:"D3-065", domain:3, obj:"3.3", diff:"hard", q:"A high-traffic website wants clients to verify its certificate is not revoked without each client contacting the CA, reducing latency and privacy leakage. Which solution BEST addresses this?", options:["Publishing a larger CRL","OCSP stapling","Disabling revocation checking","Using a self-signed certificate"], answer:1, explain:"<strong>OCSP stapling</strong> has the web server attach a time-stamped OCSP response during the TLS handshake, so clients verify revocation without contacting the CA themselves.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Publishing a larger CRL</b>: a bigger CRL increases download size and latency, the opposite of the goal.</span><span class='qd'><b>Disabling revocation checking</b>: turning off revocation checking removes security rather than addressing latency safely.</span><span class='qd'><b>Using a self-signed certificate</b>: a self-signed certificate is untrusted by clients and does not solve revocation verification.</span>" },
  { id:"D3-066", domain:3, obj:"3.3", diff:"medium", q:"What is the primary purpose of key escrow?", options:["To accelerate encryption","To store a copy of cryptographic keys so data can be recovered if keys are lost","To replace certificate authorities","To generate random numbers"], answer:1, explain:"<strong>To store a copy of cryptographic keys so data can be recovered if keys are lost</strong> is the primary purpose of key escrow, held by a trusted party.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>To accelerate encryption</b>: escrow concerns key recovery, not cryptographic performance.</span><span class='qd'><b>To replace certificate authorities</b>: escrow stores keys and does not perform the CA's issuing and signing role.</span><span class='qd'><b>To generate random numbers</b>: random number generation is a separate cryptographic primitive unrelated to key escrow.</span>" },
  { id:"D3-067", domain:3, obj:"3.3", diff:"medium", q:"Which dedicated hardware device is designed to generate, store, and manage cryptographic keys with tamper resistance for enterprise use?", options:["TPM","HSM","NIC","UPS"], answer:1, explain:"<strong>HSM</strong> (hardware security module) is a dedicated, tamper-resistant appliance for enterprise key generation, storage, and cryptographic operations.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>TPM</b>: a TPM is a chip bound to a single host for that device's keys and integrity, not a dedicated enterprise-wide key appliance.</span><span class='qd'><b>NIC</b>: a network interface card handles network connectivity, not key management.</span><span class='qd'><b>UPS</b>: an uninterruptible power supply provides backup power and has no cryptographic function.</span>" },
  ...
  { id:"D3-069", domain:3, obj:"3.3", diff:"easy", q:"Which algorithm is a symmetric block cipher widely used to encrypt data at rest?", options:["RSA","AES","ECC","SHA-256"], answer:1, explain:"<strong>AES</strong> is a symmetric block cipher widely used to encrypt data at rest and in transit.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>RSA</b>: RSA is an asymmetric algorithm used for key exchange and signatures, not a symmetric block cipher.</span><span class='qd'><b>ECC</b>: ECC is asymmetric elliptic curve cryptography, not a symmetric block cipher.</span><span class='qd'><b>SHA-256</b>: SHA-256 is a one-way hash function, not an encryption cipher.</span>" },
  { id:"D3-070", domain:3, obj:"3.3", diff:"medium", q:"Why is ECC often preferred over RSA for mobile and IoT devices?", options:["ECC requires no keys","ECC provides comparable security with smaller key sizes and lower computational overhead","ECC is a hashing algorithm","ECC does not use mathematics"], answer:1, explain:"<strong>ECC provides comparable security with smaller key sizes and lower computational overhead</strong>, making it efficient for resource-constrained mobile and IoT devices.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>ECC requires no keys</b>: ECC is key-based cryptography; it absolutely uses keys.</span><span class='qd'><b>ECC is a hashing algorithm</b>: ECC is an asymmetric encryption and signature scheme, not a hash function.</span><span class='qd'><b>ECC does not use mathematics</b>: ECC is grounded in elliptic curve mathematics, so this is false.</span>" },
  ...
  { id:"D3-072", domain:3, obj:"3.3", diff:"medium", q:"Which construct combines a hash function with a secret key to provide both integrity and authentication of a message?", options:["HMAC","CRL","CSR","OCSP"], answer:0, explain:"<strong>HMAC</strong> combines a hash function with a secret key to provide both message integrity and authentication.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>CRL</b>: a certificate revocation list lists revoked certificates and does not authenticate messages.</span><span class='qd'><b>CSR</b>: a certificate signing request asks a CA to issue a certificate, unrelated to message authentication.</span><span class='qd'><b>OCSP</b>: OCSP checks certificate revocation status, not the integrity and authenticity of a message.</span>" },
  { id:"D3-073", domain:3, obj:"3.3", diff:"hard", q:"An administrator must choose a key length and algorithm that resists brute force while remaining standard for government-grade data at rest. Which choice is MOST appropriate?", options:["DES with 56-bit keys","AES with 256-bit keys","MD5 hashing","ROT13 substitution"], answer:1, explain:"<strong>AES with 256-bit keys</strong> is a current standard offering strong brute-force resistance for government-grade data at rest.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>DES with 56-bit keys</b>: DES is deprecated and its 56-bit key is trivially brute-forced today.</span><span class='qd'><b>MD5 hashing</b>: MD5 is a weak, collision-prone hash and is not an encryption algorithm for protecting data.</span><span class='qd'><b>ROT13 substitution</b>: ROT13 is trivial obfuscation with no real cryptographic strength.</span>" },
  { id:"D3-074", domain:3, obj:"3.3", diff:"hard", q:"During a PKI compromise, a CA's private signing key is suspected stolen. What is the MOST important immediate action?", options:["Issue more certificates faster","Revoke the affected certificates and update the CRL/OCSP","Increase the key length on new requests","Switch to symmetric encryption"], answer:1, explain:"<strong>Revoke the affected certificates and update the CRL/OCSP</strong> is the most important immediate action so relying parties stop trusting compromised certificates.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Issue more certificates faster</b>: issuing more certificates from a compromised CA spreads risk rather than containing it.</span><span class='qd'><b>Increase the key length on new requests</b>: longer keys on future certificates does nothing about the already-trusted compromised ones.</span><span class='qd'><b>Switch to symmetric encryption</b>: changing encryption type does not address revoking the affected PKI certificates.</span>" },
  { id:"D3-075", domain:3, obj:"3.3", diff:"medium", q:"Which stage of the certificate lifecycle removes a certificate from trust before its scheduled expiration?", options:["Enrollment","Revocation","Renewal","Generation"], answer:1, explain:"<strong>Revocation</strong> removes a certificate from trust before its scheduled expiration, for example after a key compromise or employee departure.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Enrollment</b>: enrollment is the initial issuance stage, not the removal of trust.</span><span class='qd'><b>Renewal</b>: renewal extends a certificate's validity, the opposite of removing it from trust.</span><span class='qd'><b>Generation</b>: generation creates the key pair and certificate, not invalidating one early.</span>" },
...
SECPLUS.questions.push(
  ...
  { id:"D4-015", domain:4, obj:"4.1", diff:"easy", q:"What is the purpose of applying the principle of least functionality when hardening a host?", options:["To install as many features as possible","To provide only the functions required for the host's role","To allow every user to be an administrator","To disable all logging"], answer:1, explain:"<strong>To provide only the functions required for the host's role</strong> is the goal of least functionality, running only the services, applications, and protocols a system needs.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>To install as many features as possible</b>: maximizing features expands the attack surface, the opposite of least functionality.</span><span class='qd'><b>To allow every user to be an administrator</b>: that violates least privilege and is unrelated to limiting a host's functions.</span><span class='qd'><b>To disable all logging</b>: disabling logging removes visibility and security telemetry, which least functionality does not require.</span>" },
  { id:"D4-016", domain:4, obj:"4.1", diff:"hard", q:"A wireless engineer must choose channel widths and placement to minimize co-channel interference in a dense office. Which activity provides the data to make these decisions?", options:["A penetration test","A wireless site survey and heat map","A tabletop exercise","A data loss prevention scan"], answer:1, explain:"<strong>A wireless site survey and heat map</strong> produces signal strength, coverage, and interference data that informs access point placement, channel selection, and power settings.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>A penetration test</b>: it attempts to exploit security weaknesses, not measure RF coverage or co-channel interference.</span><span class='qd'><b>A tabletop exercise</b>: it is a discussion-based incident-response drill, not a wireless RF measurement activity.</span><span class='qd'><b>A data loss prevention scan</b>: it inspects data movement for sensitive content, not wireless signal characteristics.</span>" },
  ...
  { id:"D4-027", domain:4, obj:"4.2", diff:"easy", q:"In asset management, what is 'data sanitization'?", options:["Backing up data to the cloud","Removing data from media so it cannot be reconstructed","Compressing files to save space","Encrypting files in transit"], answer:1, explain:"<strong>Removing data from media so it cannot be reconstructed</strong> is data sanitization, achieved through methods such as overwriting, cryptographic erase, or physical destruction.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Backing up data to the cloud</b>: backup copies and preserves data, the opposite of rendering it irretrievable.</span><span class='qd'><b>Compressing files to save space</b>: compression reduces file size while keeping data fully readable.</span><span class='qd'><b>Encrypting files in transit</b>: that protects data moving over a network, not removing data from media at end of life.</span>" },
  { id:"D4-028", domain:4, obj:"4.2", diff:"hard", q:"A company discovers many unmanaged virtual machines spun up by developers and never tracked. Which asset management concept does this gap represent, and what should be implemented FIRST?", options:["Shadow IT; implement an enrollment/provisioning and inventory process","Data classification; encrypt all VMs","Patch tuesday; reboot all servers","Penetration testing; attack the VMs"], answer:0, explain:"<strong>Shadow IT; implement an enrollment/provisioning and inventory process</strong> is correct because untracked, self-provisioned resources are shadow IT, and standardized provisioning tied to the inventory brings them under management first.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Data classification; encrypt all VMs</b>: classification and encryption do not address the core problem of unknown, untracked assets.</span><span class='qd'><b>Patch tuesday; reboot all servers</b>: rebooting patched servers does not discover or inventory the untracked VMs.</span><span class='qd'><b>Penetration testing; attack the VMs</b>: exploiting the VMs does not bring them into managed inventory or governance.</span>" },
  { id:"D4-029", domain:4, obj:"4.2", diff:"medium", q:"Why is classifying assets by criticality important for security operations?", options:["It makes invoices larger","It prioritizes protection, monitoring, and recovery resources","It eliminates the need for backups","It disables auditing"], answer:1, explain:"<strong>It prioritizes protection, monitoring, and recovery resources</strong> by ensuring the most critical assets receive proportionally stronger controls and recovery priority.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>It makes invoices larger</b>: classification by criticality has nothing to do with billing amounts.</span><span class='qd'><b>It eliminates the need for backups</b>: criticality classification informs backup priority, it does not remove the need to back up data.</span><span class='qd'><b>It disables auditing</b>: classification supports stronger oversight, not the disabling of audit functions.</span>" },
  ...
  { id:"D4-043", domain:4, obj:"4.3", diff:"easy", q:"Which source provides intelligence about known exploited vulnerabilities and emerging threats to inform vulnerability prioritization?", options:["Threat intelligence feeds","The company lunch menu","A printer manual","A screensaver"], answer:0, explain:"<strong>Threat intelligence feeds</strong> deliver information about known exploited vulnerabilities and emerging threats, helping teams prioritize flaws under active exploitation.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>The company lunch menu</b>: it contains food options and no security threat data.</span><span class='qd'><b>A printer manual</b>: it documents device operation, not vulnerability or threat intelligence.</span><span class='qd'><b>A screensaver</b>: it is a display feature with no threat information.</span>" },
  { id:"D4-044", domain:4, obj:"4.3", diff:"medium", q:"What does responsible disclosure require of a researcher who finds a vulnerability?", options:["Immediately publish full exploit code publicly","Notify the vendor and allow time to remediate before public disclosure","Sell the vulnerability to attackers","Ignore the finding"], answer:1, explain:"<strong>Notify the vendor and allow time to remediate before public disclosure</strong> is what responsible disclosure requires, giving the vendor a reasonable window to patch before details go public.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Immediately publish full exploit code publicly</b>: releasing exploit code before a patch exposes users to attack and is the opposite of responsible disclosure.</span><span class='qd'><b>Sell the vulnerability to attackers</b>: that is malicious and illegal, not responsible disclosure.</span><span class='qd'><b>Ignore the finding</b>: leaving the flaw unreported allows it to persist unaddressed, helping no one.</span>" },
  { id:"D4-045", domain:4, obj:"4.3", diff:"hard", q:"A team has thousands of findings and limited staff. Which approach MOST effectively reduces overall risk with constrained resources?", options:["Patch findings alphabetically by hostname","Remediate based on risk: exploitability, exposure, and asset criticality","Patch only the newest CVEs regardless of severity","Ignore all critical findings"], answer:1, explain:"<strong>Remediate based on risk: exploitability, exposure, and asset criticality</strong> focuses limited effort on the vulnerabilities most likely to be exploited on the most important assets, maximizing risk reduction.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Patch findings alphabetically by hostname</b>: name order is arbitrary and ignores actual severity and exposure.</span><span class='qd'><b>Patch only the newest CVEs regardless of severity</b>: recency does not equal risk, and older critical flaws may pose greater danger.</span><span class='qd'><b>Ignore all critical findings</b>: skipping the most severe vulnerabilities leaves the highest-risk exposures unaddressed.</span>" },
  ...
  { id:"D4-059", domain:4, obj:"4.4", diff:"medium", q:"Which DLP deployment inspects data leaving the organization over the network, such as email and web uploads?", options:["Endpoint DLP only","Network DLP","Storage/at-rest DLP only","A vulnerability scanner"], answer:1, explain:"<strong>Network DLP</strong> monitors data in motion across egress points such as email, web, and file transfer to detect and block unauthorized transmission of sensitive data.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Endpoint DLP only</b>: it inspects activity on the device itself, such as copying to USB, not data leaving over the network.</span><span class='qd'><b>Storage/at-rest DLP only</b>: it scans stored data on servers and repositories, not data crossing the network egress.</span><span class='qd'><b>A vulnerability scanner</b>: it finds weaknesses and missing patches, not data leaving the organization.</span>" },
  { id:"D4-060", domain:4, obj:"4.4", diff:"hard", q:"A SOC wants to detect a new malware variant that no vendor signature yet matches. Which capability is MOST likely to catch it?", options:["Strict signature-only antivirus","Behavioral/heuristic EDR detection","Disabling logging","An asset inventory spreadsheet"], answer:1, explain:"<strong>Behavioral/heuristic EDR detection</strong> flags suspicious actions and patterns, catching novel malware that has no existing vendor signature.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Strict signature-only antivirus</b>: it can only match known signatures, so a brand-new variant with no signature slips past it.</span><span class='qd'><b>Disabling logging</b>: removing telemetry reduces visibility and helps nothing in detecting new malware.</span><span class='qd'><b>An asset inventory spreadsheet</b>: it tracks assets but performs no malware detection.</span>" },
  { id:"D4-061", domain:4, obj:"4.4", diff:"easy", q:"What does log aggregation accomplish in a monitoring architecture?", options:["It deletes logs to save space","It centralizes logs from many sources into one place for analysis","It encrypts the network","It assigns IP addresses"], answer:1, explain:"<strong>It centralizes logs from many sources into one place for analysis</strong>, so events can be searched, correlated, and retained consistently.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>It deletes logs to save space</b>: aggregation collects and retains logs, it does not delete them.</span><span class='qd'><b>It encrypts the network</b>: network encryption protects traffic, not a log-collection function.</span><span class='qd'><b>It assigns IP addresses</b>: address assignment is a DHCP role, unrelated to log centralization.</span>" },
  { id:"D4-062", domain:4, obj:"4.4", diff:"medium", q:"Which SNMP version should be used to ensure authentication and encryption of management traffic?", options:["SNMPv1","SNMPv2c","SNMPv3","SNMPv0"], answer:2, explain:"<strong>SNMPv3</strong> adds authentication and encryption through its security model, unlike v1 and v2c which send community strings in cleartext.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>SNMPv1</b>: it uses cleartext community strings with no authentication or encryption.</span><span class='qd'><b>SNMPv2c</b>: it still relies on cleartext community strings and lacks encryption.</span><span class='qd'><b>SNMPv0</b>: there is no SNMPv0 standard, so it is not a valid choice.</span>" },
  { id:"D4-063", domain:4, obj:"4.4", diff:"hard", q:"After deploying DLP, the team finds it blocks legitimate business emails containing customer account numbers. What is the BEST next step?", options:["Disable DLP completely","Refine DLP policies and use exceptions/contextual rules to reduce false positives","Block all outbound email permanently","Ignore the complaints"], answer:1, explain:"<strong>Refine DLP policies and use exceptions/contextual rules to reduce false positives</strong> tunes the system so legitimate business email flows while genuinely sensitive data stays protected.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Disable DLP completely</b>: turning off protection removes the control entirely and exposes sensitive data.</span><span class='qd'><b>Block all outbound email permanently</b>: halting all email cripples business operations and overcorrects the problem.</span><span class='qd'><b>Ignore the complaints</b>: leaving valid emails blocked harms the business and resolves nothing.</span>" },
  { id:"D4-064", domain:4, obj:"4.4", diff:"medium", q:"What is the purpose of a security alert's 'severity' or priority field?", options:["To indicate the analyst's name","To help responders triage which alerts to address first","To set the screen brightness","To choose the font color"], answer:1, explain:"<strong>To help responders triage which alerts to address first</strong> is the purpose of an alert's severity field, ensuring the most urgent, high-impact alerts are handled first.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>To indicate the analyst's name</b>: severity conveys urgency, not who is assigned to the alert.</span><span class='qd'><b>To set the screen brightness</b>: display brightness is unrelated to alert prioritization.</span><span class='qd'><b>To choose the font color</b>: visual styling is not the meaning of a severity field.</span>" },
  { id:"D4-065", domain:4, obj:"4.4", diff:"medium", q:"Which monitoring data source captures the full content of network packets for deep inspection?", options:["NetFlow","Packet capture (PCAP)","SNMP polling","DHCP logs"], answer:1, explain:"<strong>Packet capture (PCAP)</strong> records full packet contents, enabling deep payload inspection.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>NetFlow</b>: it records only flow metadata such as addresses and volume, not full packet payloads.</span><span class='qd'><b>SNMP polling</b>: it gathers device status and counters, not the full content of network packets.</span><span class='qd'><b>DHCP logs</b>: they record IP lease assignments, not packet payloads.</span>" },
...
SECPLUS.questions.push(
  ...
  { id:"D4-075", domain:4, obj:"4.5", diff:"medium", q:"A domain owner wants to instruct receiving servers to quarantine or reject mail that fails SPF and DKIM checks, and to receive reports. Which record provides this?", options:["DMARC","SPF","PTR","MX"], answer:0, explain:"<strong>DMARC</strong> builds on SPF and DKIM to publish a policy of none, quarantine, or reject for failing mail and to deliver aggregate and forensic reports to the domain owner.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>SPF</b>: SPF only lists authorized sending hosts and offers no enforcement policy or reporting back to the domain owner.</span><span class='qd'><b>PTR</b>: a PTR record provides reverse DNS mapping of an IP to a hostname, used for sanity checks but not for setting mail-failure policy.</span><span class='qd'><b>MX</b>: an MX record specifies which servers receive mail for a domain and has nothing to do with handling authentication failures.</span>" },
  { id:"D4-076", domain:4, obj:"4.5", diff:"medium", q:"Which tool establishes behavioral baselines for users and entities and flags statistically anomalous activity such as impossible-travel logins?", options:["UEBA","SIEM correlation rules","Antivirus signatures","A jump server"], answer:0, explain:"<strong>UEBA</strong> builds baselines of normal user and entity behavior and flags statistical deviations such as impossible-travel logins that signature tools cannot catch.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>SIEM correlation rules</b>: correlation rules match predefined logic across logs, but they detect known conditions rather than learning behavioral baselines to spot anomalies.</span><span class='qd'><b>Antivirus signatures</b>: signature matching identifies known malware files and cannot model user behavior or detect anomalous login patterns.</span><span class='qd'><b>A jump server</b>: a jump server is a hardened administrative pivot host for controlled access, not an analytics tool that detects anomalies.</span>" },
  { id:"D4-077", domain:4, obj:"4.5", diff:"hard", q:"A security team needs cross-layer correlation that combines endpoint, network, email, and cloud telemetry into a single detection and response platform. Which solution BEST fits?", options:["EDR","XDR","NAC","DLP"], answer:1, explain:"<strong>XDR</strong> unifies and correlates telemetry across endpoint, network, email, and cloud control points into a single detection and response platform, going beyond endpoint-only tools.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>EDR</b>: EDR focuses on endpoint telemetry alone and lacks the native cross-layer correlation that the scenario requires.</span><span class='qd'><b>NAC</b>: Network Access Control enforces admission and posture checks for devices joining the network, not multi-source detection and response.</span><span class='qd'><b>DLP</b>: Data Loss Prevention monitors and blocks sensitive data from leaving, which is a narrow data-protection function, not cross-layer threat correlation.</span>" },
  ...
  { id:"D4-083", domain:4, obj:"4.5", diff:"medium", q:"Which firewall type makes allow/deny decisions based on application identity and user context rather than only ports and protocols?", options:["Packet-filtering firewall","Next-generation firewall (NGFW)","Stateless ACL","Circuit-level gateway"], answer:1, explain:"<strong>Next-generation firewall (NGFW)</strong> adds deep packet inspection, application awareness, and user identity to traditional filtering, so it decides based on application and user context.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Packet-filtering firewall</b>: a packet filter decides solely on header fields like IP and port and has no awareness of applications or users.</span><span class='qd'><b>Stateless ACL</b>: a stateless access control list matches addresses and ports without tracking sessions or identifying applications.</span><span class='qd'><b>Circuit-level gateway</b>: a circuit-level gateway validates the TCP handshake at the session layer but does not inspect application identity or user context.</span>" },
  { id:"D4-084", domain:4, obj:"4.5", diff:"easy", q:"What does an agent-based EDR primarily monitor?", options:["Endpoint process, file, and registry activity","Only perimeter firewall logs","Physical badge readers","HVAC sensors"], answer:0, explain:"<strong>Endpoint process, file, and registry activity</strong> is what an agent-based EDR records and analyzes on the host to detect and respond to threats.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Only perimeter firewall logs</b>: perimeter firewall logs are network-edge data, whereas EDR agents operate on the endpoint itself.</span><span class='qd'><b>Physical badge readers</b>: badge readers are a physical access control function, not something an endpoint software agent monitors.</span><span class='qd'><b>HVAC sensors</b>: HVAC sensors report environmental conditions and are unrelated to endpoint security telemetry.</span>" },
  { id:"D4-085", domain:4, obj:"4.6", diff:"easy", q:"Which IAM process creates a new user account and grants initial access when an employee is hired?", options:["Deprovisioning","Provisioning","Recertification","Federation"], answer:1, explain:"<strong>Provisioning</strong> establishes the new identity and assigns initial entitlements as part of onboarding a hired employee.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Deprovisioning</b>: deprovisioning removes or disables access when someone leaves or changes roles, the opposite of onboarding.</span><span class='qd'><b>Recertification</b>: recertification periodically reviews whether existing access is still appropriate, not the initial account creation.</span><span class='qd'><b>Federation</b>: federation establishes trust so identities can be used across organizations and does not create a new internal account on hire.</span>" },
  { id:"D4-086", domain:4, obj:"4.6", diff:"easy", q:"Which authentication approach lets a user sign in once and access multiple applications without re-entering credentials?", options:["Single sign-on (SSO)","Multifactor authentication","Just-in-time access","Mandatory access control"], answer:0, explain:"<strong>Single sign-on (SSO)</strong> authenticates the user once and propagates that authenticated session to other trusting applications without re-entry of credentials.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Multifactor authentication</b>: MFA strengthens a single login by requiring multiple factors, but it does not let one sign-in carry across many applications.</span><span class='qd'><b>Just-in-time access</b>: JIT access grants elevated rights only temporarily when needed, addressing privilege duration rather than reusing one authentication.</span><span class='qd'><b>Mandatory access control</b>: MAC is an authorization model based on labels and clearances, not a single-authentication convenience mechanism.</span>" },
  { id:"D4-087", domain:4, obj:"4.6", diff:"medium", q:"Which protocol is an XML-based standard commonly used for web-based SSO and federation between an identity provider and a service provider?", options:["RADIUS","SAML","Kerberos","TACACS+"], answer:1, explain:"<strong>SAML</strong> exchanges XML assertions between an identity provider and a service provider to enable federated web-based single sign-on.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>RADIUS</b>: RADIUS is a network access AAA protocol commonly used for VPN and wireless authentication, not XML-based web federation.</span><span class='qd'><b>Kerberos</b>: Kerberos uses ticket-based authentication within a domain such as a LAN, not XML assertions exchanged for web SSO.</span><span class='qd'><b>TACACS+</b>: TACACS+ is a Cisco AAA protocol focused on device administration command authorization, not browser federation.</span>" },
  { id:"D4-088", domain:4, obj:"4.6", diff:"medium", q:"OAuth 2.0 is primarily a framework for what?", options:["Authentication of identities","Delegated authorization granting limited access to resources","Encrypting data at rest","Directory replication"], answer:1, explain:"<strong>Delegated authorization granting limited access to resources</strong> is the purpose of OAuth 2.0, which issues scoped access tokens without sharing the user's credentials.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Authentication of identities</b>: OAuth 2.0 is an authorization framework; identity authentication is layered on top by OpenID Connect, not by OAuth itself.</span><span class='qd'><b>Encrypting data at rest</b>: data-at-rest encryption is a cryptographic storage control unrelated to delegated access tokens.</span><span class='qd'><b>Directory replication</b>: directory replication synchronizes directory data between servers and has nothing to do with delegated authorization.</span>" },
  ...
  { id:"D4-090", domain:4, obj:"4.6", diff:"medium", q:"A login requires a password and a fingerprint. Which two MFA factor categories are combined?", options:["Something you know and something you are","Something you have and somewhere you are","Something you know and something you have","Two of the same factor"], answer:0, explain:"<strong>Something you know and something you are</strong> is correct because a password is a knowledge factor and a fingerprint is an inherence (biometric) factor, two distinct categories.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Something you have and somewhere you are</b>: neither a password nor a fingerprint is a possession token or a location, so this pairing does not match.</span><span class='qd'><b>Something you know and something you have</b>: a fingerprint is a biometric inherence factor, not a possession factor, so this mislabels the second factor.</span><span class='qd'><b>Two of the same factor</b>: a password and a fingerprint belong to different categories, so combining them is true multifactor, not two of the same.</span>" },
  { id:"D4-091", domain:4, obj:"4.6", diff:"medium", q:"A hardware security key that you plug into a USB port to authenticate represents which MFA factor?", options:["Something you know","Something you have","Something you are","Somewhere you are"], answer:1, explain:"<strong>Something you have</strong> is correct because a physical USB security key is a possession factor that the user must hold to authenticate.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Something you know</b>: a knowledge factor is a memorized secret like a password or PIN, not a physical device.</span><span class='qd'><b>Something you are</b>: an inherence factor is a biometric trait such as a fingerprint, not an object you carry.</span><span class='qd'><b>Somewhere you are</b>: a location factor is based on geolocation or network context, not on possessing a hardware key.</span>" },
  { id:"D4-092", domain:4, obj:"4.6", diff:"hard", q:"In a strict MAC environment, who determines whether a subject can access an object?", options:["The object's owner at their discretion","The system, based on security labels and clearances enforced by policy","Any administrator on request","The subject themselves"], answer:1, explain:"<strong>The system, based on security labels and clearances enforced by policy</strong> is correct because strict MAC has a central authority assign labels that the system enforces, with no owner discretion.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>The object's owner at their discretion</b>: owner-driven access is the defining trait of DAC, the opposite of mandatory access control.</span><span class='qd'><b>Any administrator on request</b>: in MAC, access follows fixed label and clearance policy, not ad hoc administrator requests.</span><span class='qd'><b>The subject themselves</b>: subjects in MAC cannot grant themselves access; clearances are assigned and enforced by the system.</span>" },
  { id:"D4-093", domain:4, obj:"4.6", diff:"medium", q:"Which access control model assigns permissions to roles, and users gain permissions by being placed into those roles?", options:["DAC","MAC","RBAC","Rule-based"], answer:2, explain:"<strong>RBAC</strong> grants permissions to roles, and users inherit those permissions through role membership, which simplifies administration.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>DAC</b>: DAC lets the resource owner grant access at their discretion rather than assigning permissions through defined roles.</span><span class='qd'><b>MAC</b>: MAC enforces access using security labels and clearances set by policy, not role membership.</span><span class='qd'><b>Rule-based</b>: rule-based access control applies global if-then conditions such as time-of-day restrictions, not permissions tied to user roles.</span>" },
  { id:"D4-094", domain:4, obj:"4.6", diff:"hard", q:"A company needs access decisions based on multiple attributes such as department, device posture, time of day, and data classification. Which model BEST supports this?", options:["DAC","RBAC","ABAC","MAC"], answer:2, explain:"<strong>ABAC</strong> evaluates policies over many attributes of the subject, resource, action, and environment, making it ideal for decisions based on department, device posture, time, and classification.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>DAC</b>: DAC bases access on owner discretion and cannot natively weigh multiple contextual attributes for each decision.</span><span class='qd'><b>RBAC</b>: RBAC ties permissions to roles only and lacks the fine-grained, multi-attribute evaluation the scenario demands.</span><span class='qd'><b>MAC</b>: MAC decides by comparing labels to clearances and does not incorporate dynamic attributes like device posture or time of day.</span>" },
  { id:"D4-095", domain:4, obj:"4.6", diff:"medium", q:"In Discretionary Access Control (DAC), who typically controls access to a resource?", options:["A central policy engine","The resource owner","A federation broker","The network firewall"], answer:1, explain:"<strong>The resource owner</strong> is correct because in DAC the owner of an object has discretion to grant or revoke access to other subjects.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>A central policy engine</b>: centralized policy-driven decisions describe models like ABAC or MAC, not owner-controlled DAC.</span><span class='qd'><b>A federation broker</b>: a federation broker mediates trust between identity providers and service providers and does not control object-level permissions.</span><span class='qd'><b>The network firewall</b>: a firewall filters network traffic and plays no role in deciding who owns or can share a file resource.</span>" },
  { id:"D4-096", domain:4, obj:"4.6", diff:"hard", q:"Which PAM feature reduces standing privilege by granting elevated rights only for a limited window when a task requires them?", options:["Password complexity policy","Just-in-time (JIT) access","Account lockout threshold","Single sign-on"], answer:1, explain:"<strong>Just-in-time (JIT) access</strong> elevates rights only for a limited window when a task requires them and revokes them automatically, shrinking standing privilege.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Password complexity policy</b>: complexity rules strengthen passwords against guessing but do nothing to limit how long elevated rights persist.</span><span class='qd'><b>Account lockout threshold</b>: lockout thresholds stop brute-force attempts by disabling accounts after failed logins, unrelated to reducing standing privilege.</span><span class='qd'><b>Single sign-on</b>: SSO streamlines authentication across applications and does not time-box or revoke elevated privileges.</span>" },
  ...
  { id:"D4-102", domain:4, obj:"4.6", diff:"medium", q:"Which factor type is represented by geolocation or network-based conditions such as 'connecting from the corporate office'?", options:["Something you know","Something you have","Something you are","Somewhere you are"], answer:3, explain:"<strong>Somewhere you are</strong> is correct because geolocation or network-based conditions such as connecting from the corporate office represent the location factor.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Something you know</b>: a knowledge factor is a memorized secret like a password, not a physical or network location.</span><span class='qd'><b>Something you have</b>: a possession factor is a device or token the user holds, not where they are connecting from.</span><span class='qd'><b>Something you are</b>: an inherence factor is a biometric trait, not a geographic or network condition.</span>" },
  { id:"D4-103", domain:4, obj:"4.6", diff:"medium", q:"A TOTP code from an authenticator app changes every 30 seconds. Which MFA factor does it represent?", options:["Something you know","Something you have","Something you are","Somewhere you are"], answer:1, explain:"<strong>Something you have</strong> is correct because a TOTP code is generated by a registered device or app the user possesses, making it a possession factor.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Something you know</b>: the TOTP value is not memorized but generated by a device, so it is not a knowledge factor.</span><span class='qd'><b>Something you are</b>: a biometric trait is inherence, whereas a one-time code comes from a possessed device, not the user's body.</span><span class='qd'><b>Somewhere you are</b>: a location factor depends on geography or network, not on holding the device that produces the code.</span>" },
  { id:"D4-104", domain:4, obj:"4.7", diff:"medium", q:"A SOC builds an automated playbook that enriches an alert, opens a ticket, and isolates the host without analyst intervention. Which platform enables this?", options:["SOAR","SIEM only","DLP","NAC"], answer:0, explain:"<strong>SOAR</strong> executes playbooks that automate enrichment, ticketing, and response actions such as host isolation without analyst intervention.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>SIEM only</b>: a SIEM aggregates and correlates logs to alert, but on its own it lacks the orchestrated automated response actions the playbook performs.</span><span class='qd'><b>DLP</b>: DLP detects and blocks sensitive data exfiltration and does not orchestrate multi-step incident response workflows.</span><span class='qd'><b>NAC</b>: NAC controls device admission to the network and cannot enrich alerts or run cross-tool response playbooks.</span>" },
  { id:"D4-105", domain:4, obj:"4.7", diff:"easy", q:"What is a primary benefit of automating repetitive security operations tasks?", options:["Slower, more deliberate responses","Reduced human error and faster, consistent response","Elimination of all monitoring needs","Higher false-positive rates"], answer:1, explain:"<strong>Reduced human error and faster, consistent response</strong> is correct because automation enforces speed and consistency while eliminating mistakes from repetitive manual work.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Slower, more deliberate responses</b>: automation speeds response, so slowing it down is the opposite of the benefit.</span><span class='qd'><b>Elimination of all monitoring needs</b>: automation handles tasks but still requires oversight and tuning, so monitoring is not eliminated.</span><span class='qd'><b>Higher false-positive rates</b>: well-designed automation does not inherently raise false positives, and higher false positives would be a drawback, not a benefit.</span>" },
...
{ id:"D5-016", domain:5, obj:"5.1", diff:"hard", q:"Leadership wants a single statement that expresses how much risk the enterprise is willing to pursue in order to meet objectives, to guide all downstream policies. This is the organization's:", options:["Risk tolerance","Risk appetite","Risk register","Risk threshold"], answer:1, explain:"<strong>Risk appetite</strong> is the broad amount and type of risk an organization is willing to pursue in order to meet its objectives, guiding downstream policy.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Risk tolerance</b>: tolerance is the acceptable variation around a specific risk objective, narrower than the enterprise-wide appetite.</span><span class='qd'><b>Risk register</b>: a risk register is a tracking repository of identified risks, not a statement of how much risk the enterprise will accept.</span><span class='qd'><b>Risk threshold</b>: a threshold is a specific trigger point for action on an individual risk, not the overarching willingness-to-accept statement.</span>" },
{ id:"D5-017", domain:5, obj:"5.1", diff:"easy", q:"Which of the following is the MOST authoritative and mandatory in a typical document hierarchy?", options:["Guideline","Procedure","Standard","Policy"], answer:3, explain:"A <strong>policy</strong> sits at the top of the document hierarchy as the mandatory management directive that standards and procedures support.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Guideline</b>: guidelines are advisory and non-binding, placing them at the least authoritative level.</span><span class='qd'><b>Procedure</b>: procedures are mandatory but subordinate, implementing the steps required to satisfy policy.</span><span class='qd'><b>Standard</b>: standards are mandatory yet derive their authority from the policy they enforce, so they rank below it.</span>" },
...
{ id:"D5-043", domain:5, obj:"5.2", diff:"medium", q:"Which metric represents the average time required to restore a failed component to operation?", options:["MTBF","MTTR","RPO","ARO"], answer:1, explain:"<strong>MTTR</strong> (Mean Time To Repair) is the average time required to restore a failed component to operation.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>MTBF</b>: MTBF measures the average interval between failures, indicating reliability, not repair time.</span><span class='qd'><b>RPO</b>: an RPO is the acceptable amount of data loss in time, not a repair-time metric.</span><span class='qd'><b>ARO</b>: ARO is the annualized rate of occurrence of a threat, used in risk math, not a measure of restoration time.</span>" },
{ id:"D5-044", domain:5, obj:"5.2", diff:"medium", q:"A high Mean Time Between Failures (MTBF) value indicates that a component is:", options:["Failing very frequently","Reliable, with long intervals between failures","Quick to repair after failure","Expensive to replace"], answer:1, explain:"A high <strong>reliable, with long intervals between failures</strong> value means the component runs longer on average before failing, indicating greater reliability.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Failing very frequently</b>: frequent failure would produce a low MTBF, not a high one.</span><span class='qd'><b>Quick to repair after failure</b>: repair speed is measured by MTTR, not by the time between failures.</span><span class='qd'><b>Expensive to replace</b>: cost of replacement is unrelated to MTBF, which measures failure intervals.</span>" },
{ id:"D5-045", domain:5, obj:"5.2", diff:"hard", q:"A critical application has an RTO of 1 hour, but the documented recovery procedure consistently takes 4 hours to complete. The FIRST priority should be to:", options:["Lower the RTO to match the 4-hour reality","Improve recovery capabilities to meet the 1-hour RTO or formally revise the objective with management","Increase backup frequency to fix the RTO","Transfer the risk through insurance"], answer:1, explain:"Because the RTO is business-driven, the first priority is to <strong>improve recovery capabilities to meet the 1-hour RTO or formally revise the objective with management</strong>.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Lower the RTO to match the 4-hour reality</b>: unilaterally lowering the objective to fit current performance abandons the business requirement without management approval.</span><span class='qd'><b>Increase backup frequency to fix the RTO</b>: backup frequency governs the RPO (data loss), not the time needed to restore service.</span><span class='qd'><b>Transfer the risk through insurance</b>: insurance offsets financial loss but does not shorten recovery time to meet the RTO.</span>" },
...
SECPLUS.questions.push(
  ...
  { id:"D5-086", domain:5, obj:"5.5", diff:"easy", q:"During the early phase of a penetration test, the tester gathers publicly available information about the target. What is this phase called?", options:["Reconnaissance","Exploitation","Reporting","Remediation"], answer:0, explain:"<strong>Reconnaissance</strong> is the early information-gathering phase where the tester collects data about the target before active testing.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Exploitation</b>: exploitation is the later phase of actively leveraging a vulnerability, not initial information gathering.</span><span class='qd'><b>Reporting</b>: reporting is the final phase documenting findings, not the early data-collection stage.</span><span class='qd'><b>Remediation</b>: remediation is fixing identified issues after testing, not the gathering phase.</span>" },
  { id:"D5-087", domain:5, obj:"5.5", diff:"medium", q:"A tester reads the target's public website, social media, and DNS records without sending packets to the target's systems. Which reconnaissance type is this?", options:["Passive reconnaissance","Active reconnaissance","Exploitation","Privilege escalation"], answer:0, explain:"<strong>Passive reconnaissance</strong> gathers information from public sources like websites, social media, and DNS records without sending packets to the target.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Active reconnaissance</b>: active reconnaissance directly interacts with target systems, such as scanning, which this scenario explicitly avoids.</span><span class='qd'><b>Exploitation</b>: exploitation actively attacks a vulnerability, not passively collecting public data.</span><span class='qd'><b>Privilege escalation</b>: privilege escalation gains higher access after compromise, not an information-gathering technique.</span>" },
  { id:"D5-088", domain:5, obj:"5.5", diff:"medium", q:"A tester runs port scans and sends probes directly to target hosts to enumerate services. Which reconnaissance type is this?", options:["Active reconnaissance","Passive reconnaissance","Attestation","Compliance monitoring"], answer:0, explain:"<strong>Active reconnaissance</strong> directly interacts with the target, such as running port scans and probing hosts to enumerate services.<br><span class='qwhy'>Why the other options are wrong:</span><span class='qd'><b>Passive reconnaissance</b>: passive reconnaissance gathers public data without touching the target, whereas scanning sends probes to it.</span><span class='qd'><b>Attestation</b>: attestation is an auditor's conformance statement, not a reconnaissance technique.</span><span class='qd'><b>Compliance monitoring</b>: compliance monitoring verifies regulatory adherence, unrelated to scanning a target.</span>" },
...
SECPLUS.pbqs.push(
  {
    ...
    fields:[
      { label:"Rule 1 ACTION (allow inbound web traffic)", hint:"Clients must reach the site.", options:["Deny","Permit","Log only","Reject with reset"], answer:1, explain:"Inbound HTTPS to the public web server must be explicitly permitted before the implicit deny." },
      { label:"Rule 1 DESTINATION PORT", hint:"HTTPS only.", options:["80","22","443","3389"], answer:2, explain:"HTTPS uses TCP 443; HTTP (80) was not requested and should stay closed." },
      { label:"Rule 1 PROTOCOL", options:["UDP","ICMP","TCP","GRE"], answer:2, explain:"HTTPS rides on TCP, so the rule must specify TCP." },
      { label:"Purpose of Rule 2 (DMZ host to internal LAN = deny)", hint:"Think about a compromised DMZ host.", options:["Improves web performance","Prevents lateral movement from a compromised DMZ host into the LAN","Blocks DNS resolution","Enables clustering"], answer:1, explain:"A DMZ host should never initiate inbound connections to the trusted LAN; denying it limits lateral movement if the server is breached." }
    ],
  ...
  },
  {
    ...
    fields:[
      { label:"Rule 1 management PORT (Linux servers)", hint:"Secure shell.", options:["23","22","21","25"], answer:1, explain:"SSH (TCP 22) is the secure remote management port; Telnet (23) is plaintext and must not be used." },
      ...
      { label:"Best practice this enforces", options:["Defense in depth via a single choke point (jump host)","Load balancing","Geofencing of DNS","Port knocking"], answer:0, explain:"Funneling all admin access through one hardened, monitored bastion is a controlled choke point reducing attack surface." },
      { label:"Why deny direct Internet RDP (3389) to servers", hint:"3389 is heavily scanned.", options:["RDP is faster than SSH","Exposed 3389 is a top ransomware entry vector and should never face the Internet","RDP uses UDP only","It conserves IP addresses"], answer:1, explain:"Internet-exposed RDP (3389) is constantly brute-forced and a leading ransomware entry point; it must be reachable only via the bastion/VPN." }
    ],
  ...
  },
  {
    ...
    fields:[
      { label:"Rule 1 DATABASE PORT (MySQL/MariaDB)", hint:"Standard MySQL listener.", options:["1433","3306","5432","1521"], answer:1, explain:"MySQL/MariaDB listens on TCP 3306; 1433 is MSSQL and 5432 is PostgreSQL." },
      ...
      { label:"Security principle demonstrated", options:["Least privilege / micro-segmentation","Fail-open design","Round-robin DNS","Split tunneling"], answer:0, explain:"Allowing only one source to one port on the DB VLAN is least privilege enforced through segmentation." }
    ],
  ...
  },
  {
    ...
    fields:[
      { label:"Rule 1 PORT (public web over TLS)", options:["8080","443","3389","21"], answer:1, explain:"Public web traffic should be HTTPS on TCP 443 from anywhere." },
      { label:"Rule 2 PORT (admin access)", hint:"Restricted management.", options:["22","80","443","123"], answer:0, explain:"SSH (TCP 22) is the admin port and is scoped to the single corporate /32 source." },
      { label:"Why scope SSH to /32 instead of 0.0.0.0/0", options:["Saves bandwidth","Least privilege: limits the SSH attack surface to one known source","Required for TLS","Enables IPv6"], answer:1, explain:"Restricting SSH to a single known IP applies least privilege and drastically shrinks the brute-force attack surface." },
      { label:"In cloud SGs, what enforces the final 'deny'?", hint:"SGs are stateful allow-lists.", options:["An explicit deny rule you must add","The implicit deny: anything not allowed is dropped","A NACL only","Nothing; all is allowed"], answer:1, explain:"Security groups are allow-lists; any traffic not explicitly allowed is implicitly denied." }
    ],
  ...
  },
  {
    ...
    fields:[
      ...
      { label:"Best-practice model for this design", options:["Flat network","Purdue model / zoned segmentation between IT and OT","Single broadcast domain","DMZ-less design"], answer:1, explain:"Separating IT and OT into zones with controlled conduits reflects the Purdue reference model for ICS security." },
      { label:"Risk if OT and IT share one flat VLAN", options:["Lower latency only","Malware in IT can pivot directly to PLCs, risking safety and uptime","Better compliance","No risk"], answer:1, explain:"A flat network lets commodity malware reach fragile control systems, threatening physical safety and availability." }
    ],
  ...
  },
  {
    ...
    fields:[
      { label:"Rule 1 PROTOCOL for DNS queries", hint:"DNS primarily uses this transport.", options:["TCP only","UDP (and TCP for large responses)","ICMP","GRE"], answer:1, explain:"DNS uses UDP 53 for most queries (TCP 53 for zone transfers/large responses); permit it to the internal resolver." },
      ...
      { label:"This control is best described as", options:["DNS exfiltration prevention via egress filtering","Ingress filtering","Port forwarding","NAT hairpinning"], answer:0, explain:"Restricting outbound DNS to a sanctioned resolver is egress filtering aimed at preventing DNS-based exfiltration." },
      { label:"Indicator suggesting DNS tunneling", options:["Short A-record lookups to known sites","Long, high-entropy subdomain queries at high volume to one domain","NTP sync traffic","ARP requests"], answer:1, explain:"Frequent, lengthy, high-entropy subdomain queries to a single domain are a classic sign of DNS tunneling/exfiltration." }
    ],
  ...
  },
  {
    ...
    fields:[
      { label:"Rule 1 inbound SMTP PORT", options:["110","25","143","993"], answer:1, explain:"SMTP delivery uses TCP 25; the relay must accept it from the Internet." },
      { label:"Rule 2 purpose", options:["Lets the relay forward mail only to the internal Exchange host","Permits web traffic","Opens RDP","Allows DNS"], answer:0, explain:"The relay forwards accepted mail to exactly one internal mail store on SMTP 25, limiting its reach." },
      { label:"Rule 3 ACTION (relay -> rest of internal LAN)", hint:"Constrain a possibly compromised relay.", options:["Permit","Deny","Permit logged","Forward"], answer:1, explain:"Beyond the one mail host, the relay should not reach any internal system; deny limits blast radius if compromised." },
      { label:"Why place the relay in the DMZ rather than the LAN", options:["For faster mail","To expose an Internet-facing service while isolating it from the trusted LAN","To avoid licensing","Because DMZ has more bandwidth"], answer:1, explain:"Internet-facing services belong in the DMZ so a compromise does not directly land an attacker on the internal network." }
    ],
  ...
  },
  {
    ...
    fields:[
      { label:"Rule 1 PORT (cloud management)", options:["80","443","23","8883"], answer:1, explain:"The devices reach their cloud manager over HTTPS (TCP 443); plaintext alternatives should be avoided." },
      ...
      { label:"Primary reason IoT gets its own VLAN", options:["Cheaper switches","To segment frequently-vulnerable devices away from sensitive systems","Higher throughput","To enable PoE"], answer:1, explain:"IoT gear is commonly insecure; isolating it on its own segment limits the damage of a compromise." },
      { label:"After Rule 1 and 2, IoT Internet access to other sites is", options:["Allowed to everything","Blocked by the implicit deny (only 443 to cloud-mgmt is permitted)","Allowed on port 80","Unrestricted"], answer:1, explain:"Only the explicit permit to the cloud manager survives; all other destinations hit the implicit deny." }
    ],
  ...
  },
  {
    ...
    fields:[
      { label:"Why does Rule 2 never match?", options:["Wrong port","Rule 1 denies all sources first, shadowing the later permit","TCP is invalid","Subnet mask error"], answer:1, explain:"Because evaluation is first-match top-down, the broad Rule 1 deny matches everything and shadows the specific permit below it." },
      ...
      { label:"New Rule 1 should be (most specific deny)", options:["DENY 198.51.100.66/32 -> 10.10.7.10 any","PERMIT any any","DENY 0.0.0.0/0 any","PERMIT 0.0.0.0/0 443"], answer:0, explain:"Block the single known-bad host explicitly and first, so it is dropped before the broader permit." },
      { label:"General ACL ordering principle", options:["Broadest rules first","Most specific rules first, broadest/deny-all last","Random order is fine","Permits always last"], answer:1, explain:"Order from most specific to least specific, ending with the implicit/explicit deny-all to avoid shadowed rules." }
    ],
  ...
  },
  {
    ...
    fields:[
      { label:"Rule 1 PROTOCOL for IKE negotiation on 500", options:["TCP","UDP","ICMP","ESP"], answer:1, explain:"IKE key exchange uses UDP 500 (and UDP 4500 for NAT-T)." },
      { label:"Rule 2 protocol ESP refers to", options:["Encapsulating Security Payload (the IPsec data protocol)","Extended SMTP","Ethernet Spanning Protocol","Encrypted SSH Proxy"], answer:0, explain:"ESP (IP protocol 50) carries the encrypted IPsec payload between the tunnel endpoints." },
      { label:"Which extra port is needed when NAT is between the peers?", options:["UDP 4500 (NAT-T)","TCP 443","UDP 53","TCP 22"], answer:0, explain:"NAT Traversal encapsulates ESP in UDP 4500 so IPsec survives NAT devices." },
      { label:"Scope of the VPN permits", options:["From any source to the peer","Only between the two specific peer public IPs","From the whole Internet","From LAN-A broadcast"], answer:1, explain:"Tunnel rules should be scoped to the exact peer IPs, not any source, to minimize exposure." }
    ],
  ...
  },
  {
    ...
    fields:[
      { label:"Rule 1 ACTION (guest -> corporate)", hint:"This rule must come first.", options:["Permit","Deny","Forward","NAT"], answer:1, explain:"Block guests from all corporate subnets before permitting Internet access, preventing access to internal resources." },
      ...
      { label:"Why must the corporate-deny rule precede the Internet-permit", options:["Performance","First-match evaluation: a 0.0.0.0/0 permit would otherwise include corporate ranges","To save IPs","No reason"], answer:1, explain:"Since 0.0.0.0/0 includes RFC1918 corporate ranges, the specific corporate deny must be evaluated first." },
      { label:"Security goal of guest isolation", options:["Faster guest speeds","Prevent untrusted guest devices from touching internal/sensitive systems","Reduce DNS load","Enable printing"], answer:1, explain:"Guest networks isolate untrusted devices so they cannot reach or attack internal corporate systems." }
    ],
  ...
  },
  {
    ...
    fields:[
      ...
      { label:"Security benefit of a WAF reverse proxy", options:["Caches DNS","Inspects/filters HTTP attacks and hides the backend's identity","Speeds up SSH","Provides DHCP"], answer:1, explain:"A WAF inspects HTTP for injection/XSS and shields the backend, which is never exposed directly." },
      { label:"This pattern is an example of", options:["Flat trust","Defense in depth with a screened DMZ tier","Single-factor design","Open relay"], answer:1, explain:"Layering a DMZ WAF in front of an isolated backend adds an inspection layer and enforces segmentation." }
    ],
  ...
  },
  {
    ...
    fields:[
      { label:"Rule 3 ACTION (workstation -> any other Internet)", hint:"Default egress posture.", options:["Permit","Deny","NAT","Mirror"], answer:1, explain:"A default-deny egress drops any outbound traffic not explicitly permitted, blocking arbitrary C2 channels." },
      ...
      { label:"How default-deny egress hurts malware", options:["It encrypts payloads","It blocks unsanctioned outbound ports/destinations used for beaconing","It speeds boot time","It rotates keys"], answer:1, explain:"Most malware needs to call out; restricting egress to approved services/ports denies its command-and-control." },
      { label:"Rule 3 PORT field for the catch-all block", options:["443 only","any","53 only","8080 only"], answer:1, explain:"The catch-all egress deny applies to any port/destination not previously permitted." }
    ],
  ...
  },
  {
    ...
    fields:[
      { label:"Rule 1 PORT for SFTP (SSH File Transfer)", options:["21","20","22","69"], answer:2, explain:"SFTP runs over SSH on TCP 22; this is the secure transfer path partners should use." },
      ...
      { label:"Rule 3 ACTION (server -> internal LAN)", options:["Permit","Deny","Forward","NAT"], answer:1, explain:"An Internet-facing transfer server in a screened subnet must not initiate connections into the trusted LAN." },
      { label:"Why prefer SFTP over FTP here", options:["FTP is faster","SFTP encrypts credentials and data in transit; FTP sends them in plaintext","FTP uses fewer ports","SFTP needs no auth"], answer:1, explain:"SFTP encrypts authentication and data over SSH, whereas FTP exposes credentials and files in cleartext." }
    ],
  ...
  },
  {
    ...
    exhibitTitle2:"",
    fields:[
      { label:"Rule 4 ACTION (final catch-all)", hint:"Deny-by-default posture.", options:["Permit","Deny (preferably with logging)","Forward","Translate"], answer:1, explain:"The final rule must be an explicit deny-all, ideally logged, to enforce deny-by-default and capture anomalies." },
      ...
      { label:"Best reason to log the deny-all rule", options:["To speed traffic","To gain visibility into blocked/attempted unauthorized traffic","To reduce CPU","To disable the firewall"], answer:1, explain:"Logging denied traffic provides detection value and audit evidence of attempted policy violations." },
      { label:"With Rule 4 in place, unlisted inbound RDP (3389) is", options:["Allowed","Denied by the catch-all","Redirected","Permitted from admin"], answer:1, explain:"Any service not explicitly permitted, including RDP 3389, is dropped by the final deny-all." },
      { label:"Overall posture this ACL implements", options:["Allow-by-default","Deny-by-default with least-privilege exceptions","Open firewall","Stateless permit-all"], answer:1, explain:"Explicit permits for required services plus a final deny-all is the deny-by-default, least-privilege model." }
    ],
