/* ============================================================================
   ...
   Architecture
   ------------
   - contentData.js loads first and defines the global `SECPLUS` namespace with
...
(function () {
  ...
  var Store = (function () {
    ...
    return {
      ...
      get: function (k, fb) { return read("secplus." + k, fb); },
      set: function (k, v) { write("secplus." + k, v); },
      del: function (k) { try { localStorage.removeItem("secplus." + k); } catch (e) {} }
    };
  })();
  ...
  function renderDashboard() {
    ...
    b1.appendChild(el("p.sectsub", { text: "Start here. Understand how the SY0-701 is scored and delivered, and where the credential fits in an IT/cybersecurity career." }));
    var g1 = el("div.grid");
    g1.appendChild(card({
      icon: "🎓", tag: "OVERVIEW", title: "Exam Mechanics", cls: "",
      desc: "Format, timing, scoring, and question types for the SY0-701 &mdash; the high-stakes details every candidate should internalize before sitting the test.",
    ...
    g1.appendChild(card({
      ...
      desc: "Where Security+ sits on the career ladder: DoD 8140/8570 IAT Level II baseline, and the SOC analyst, sysadmin, and security specialist roles it unlocks.",
    ...
    if (window.FLASH) {
      ...
      gf2.appendChild(card({
        ...
        desc: "Every Security+ acronym across all five domains in one deck &mdash; the highest-yield rote material on the exam.",
    ...
    b5.appendChild(el("p.sectsub", { html: "A realistic, full-length simulation of the SY0-701 &mdash; 90 questions weighted to the official domain percentages. Sit it under timed conditions to benchmark your readiness." }));
    var g3c = el("div.grid");
    ...
    g5.appendChild(card({
      ...
      desc: "Vetted, free study tools: Professor Messer's full SY0-701 course, the official CompTIA objectives PDF, and trusted community guides.",
  ...
  /* ===========================================================================
     ...
     =========================================================================== */
  function certVer() { var v = window.SECPLUS_VER || ""; return v ? "?v=" + v : ""; }
  ...
  /* Lazily fetch a domain's reading module the first time it's opened, so the
     ...
     script populates SECPLUS.reading[id]; once cached it renders instantly. */
  ...
  function loadDomainModule(id, onReady, onFail) {
    ...
    s.src = "assets/js/content/domain" + id + ".js" + (window.SECPLUS_VER ? "?v=" + window.SECPLUS_VER : "");
    s.async = true;
    s.onload = function () {
  ...
  function openDomain(id) {
    ...
    function render() {
      ...
      Router.replace(readerView("Domain " + id, d.title, "SY0-701 Domain " + id + " · " + d.weight + "% of the exam", sections));
    }
  ...
  function openExamMechanics() { Router.open(readerView("Exam mechanics", "Exam Mechanics: SY0-701", "How the exam is built, delivered, and scored", S.examMechanics || [])); }
  function openCareer() { Router.open(readerView("Career guidance", "Career Guidance", "Where Security+ fits in your path", S.careerGuidance || [])); }
